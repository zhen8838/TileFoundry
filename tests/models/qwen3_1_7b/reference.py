...
`config.json`, through `Qwen3Config` -- so the oracle and the kernels are the same
...
def build_layer(seed=0, device="cpu", dtype=DTYPE):
    """One `Qwen3DecoderLayer` with random weights at a fixed seed.
    ...
    return oracle.randomised(
        lambda: Qwen3DecoderLayer(CONFIG, layer_idx=0), seed, device, dtype
    )
...
def build_decoder(seed=0, device="cpu", dtype=DTYPE):
    ...
    A `Qwen3ForCausalLM` rather than the base model: the decoder's own boundary is
    still hidden states in and hidden states out, but the root's weights include
    ...
    return oracle.randomised(lambda: Qwen3ForCausalLM(CONFIG), seed, device, dtype)
...
def _rope_at(rows: int, device="cpu"):
    ...
    return oracle.rope_caches(Qwen3RotaryEmbedding, CONFIG, rows, device, DTYPE)
...
def attention_reference(layer, hidden_ctx, hidden_new):
    """Hugging Face's own attention output, through `o_proj`, at the decoded
    position -- the boundary the fused `self_attention` kernel ends at."""
    ...
    with torch.no_grad():
        out, _ = layer.self_attn(
            layer.input_layernorm(sequence),
            position_embeddings=(cos.unsqueeze(0), sin.unsqueeze(0)),
            attention_mask=oracle.causal_mask(total, sequence.device.type, sequence.dtype),
        )
